...
//    $rsc = $pdo->query
   ...
   //For Contributor Table

    $rsc = $pdo->query("SELECT Contributor.contributor_id, Contributor.name, GROUP_CONCAT(DISTINCT Contributed.song_id)
			AS SongID
			FROM Contributor JOIN Contributed ON Contributor.contributor_id = Contributed.contributor_id
                    GROUP BY Contributor.contributor_id, Contributor.name");
		//GROUP_CONCAT(DISTINCT Contributed.song_id FROM Contributed);");
